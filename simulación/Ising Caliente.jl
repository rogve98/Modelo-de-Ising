...
function redSpinesCaliente(N::Int,f::Float64)
	 red = ones(Int,N,N)
    for i in 1:N
        for j in 1:N
            if rand() <= f
...
function vecinos(m::MicroEstados,i::Int)
    ...
    return [m.σ[arriba],m.σ[derecha],m.σ[abajo],m.σ[izquierda]]#,[m.frio[arriba],m.frio[derecha],m.frio[abajo],m.frio[izquierda]]
end
...
# A PARTIR DE AQUI VAMOS A HACER METROPOLIS PARA LA CONFIGURACIÓN CALIENTE
...
function simulacionMontecarlo(m::MicroEstados,T,n::Int)
    ...
    energiaCaliente[1] = energiaTotal(m)
